...
class PioEncoder{
 ...
 private:
  static uint offset;             // TODO: made private by [self].  Must test if it breaks code.
  static bool not_first_instance; // TODO: made private by [self].  Must test if it breaks code.
  uint8_t pin;
  uint sm;
  PIO pio;
  int max_step_rate;
