...
// uint PioEncoder::offset;                 // TODO: Removed by [self].  Must test if it breaks code.
// bool PioEncoder::not_first_instance;     // TODO: Removed by [self].  Must test if it breaks code.
...
PioEncoder::PioEncoder(const uint8_t pin, const bool flip, const int zero_offset, const uint8_t count_mode, PIO pio, const uint sm, const int max_step_rate){
    // static uint offset;  // TODO: Removed by Kyle Vu [self].  Test if this breaks the code.
    this->pin = pin;
    this->pio = pio;
    this->sm = sm;
